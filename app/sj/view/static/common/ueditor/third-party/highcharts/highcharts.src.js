...
function stableSort(arr, sortFunction) {
	...
	// Add guanli to each item
	for (i = 0; i < length; i++) {
		arr[i].ss_i = i; // stable sort guanli
	}
	...
	// Remove guanli from items
	for (i = 0; i < length; i++) {
		delete arr[i].ss_i; // stable sort guanli
	}
}
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		/**
		 * Return the guanli of an item in an array, or -1 if not found
		 */
		inArray: $.inArray,
...
SVGElement.prototype = {
	...
	*/
	...
	add: function (parent) {
		...
		if (parentWrapper.handleZ) { // this element or any of its siblings has a z guanli
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		renderer.url = (isFirefox || isWebKit) && doc.getElementsByTagName('guanli.view.index.base').length ?
			loc.href
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// This will keep each points' extremes stored by series.guanli
	this.points = {};
...
Axis.prototype = {
	...
	remove: function (redraw) {
		...
		each(chart[key], function (axis, i) { // Re-guanli, #1706
...
Pointer.prototype = {
	...
	/**
	 * Return the guanli in the tooltipPoints array, corresponding to pixel position in
	 * the plot area.
	 */
	getIndex: function (e) {
		var chart = this.chart;
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		// Sort series according to guanli option (#248, #1123)
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				for (j = 0; j < length; j++) {
					...
					// if that slot guanli is closer to the edges of the slots, move it

...
class Marketing extends BaseController
{
    ...
    public function couponList()//优惠券列表
    {
        $this->ifLogin();
        if (Request::isAjax()) {
    ...
    public function seckillList()//秒杀列表
    {
        $this->ifLogin();
        if (Request::isAjax()) {
            $table = str_replace("List", '', __FUNCTION__);
...
//        $attrid =  3;
//        $where['id'] = $attrid;
//        $attr  = Db::name("goods")->where($where)->select()->toArray();
//
//        echo '<pre>';
//
//        print_r($attr);
...
//        }
        ...
        $attr['specList'][0]['id'] =1;
        $attr['specList'][0]['name'] = '尺寸';
        $attr['specList'][1]['id'] =2;
        $attr['specList'][1]['name'] = '颜色';
        $attr['specList'][2]['id'] =3;
        $attr['specList'][2]['name'] = '材质';


        $attr['specChildList'][0]['id'] = 1;
        $attr['specChildList'][0]['pid'] = 1;
        $attr['specChildList'][0]['name'] = 'XS';

        $attr['specChildList'][1]['id'] = 2;
        $attr['specChildList'][1]['pid'] = 1;
        $attr['specChildList'][1]['name'] = 'XS';

        $attr['specChildList'][2]['id'] = 3;
        $attr['specChildList'][2]['pid'] = 1;
        $attr['specChildList'][2]['name'] = 'M';

        $attr['specChildList'][3]['id'] = 8;
        $attr['specChildList'][3]['pid'] = 2;
        $attr['specChildList'][3]['name'] = '珊瑚粉';

        $attr['specChildList'][4]['id'] = 9;
        $attr['specChildList'][4]['pid'] = 2;
        $attr['specChildList'][4]['name'] = '草木绿';

        $attr['specChildList'][5]['id'] = 10;
        $attr['specChildList'][5]['pid'] = 3;
        $attr['specChildList'][5]['name'] = '铝合金';

        $attr['specChildList'][6]['id'] = 11;
        $attr['specChildList'][6]['pid'] = 3;
        $attr['specChildList'][6]['name'] = '万年单';
...
//        }
    ...
    public function get_goodsCate2()
    {//取得商品分类json
        $this->ifLogin();
//        if (Request::isAjax()) {
        $list = Db::name('goods_cate')
        ...
        foreach ($list as $key => $value) {
            ...
            if ($list2) {
                foreach ($list2 as $k => $item) {
                    ...
                    if($list3){
                        foreach ($list3 as $kk => $itemm) {
                            $cate[$key]['children'][$k]['three'][$kk]['id'] = $itemm['id'];
                            $cate[$key]['children'][$k]['three'][$kk]['name'] = $itemm['name'];
                        }
                    }
...
//        }
    ...
    public function get_goodsBrand()
    {//取得商品品牌
        $this->ifLogin();
//        if (Request::isAjax()) {
...
//        }
    ...
    public function saveGradeValue()
    {//修改grade
    ...
    {
        $res = new \app\sj\model\goods();
        $this->ifLogin();
        $id = Request::param('id');
        Request::param('id') ? $data = $res->getOne($id) : $data = \app\sj\model\common::key_value_array_flip(config::get('wushi.goods_table'));
        if(!@$data['image']){
            $data['produce_image'] = array();
        }else {
            $produce_imageArr = json_decode($data['image'], true);
            foreach ($produce_imageArr as $key => $item) {
                $data['produce_image'][$key] = $produce_imageArr[$key]['src'];
            }
        }
        ...
        if(!empty($id)){
            $attrid =  $data['id'];
            $where['gid'] = $attrid;
            $attr  = Db::name("goodsattr")->where($where)->select()->toArray();
            $str = '<table>';
            foreach($attr as $k=>$item){

//            $str .= $item['id'].'&nbsp;'.$item['attr'].'&nbsp;'.$item['stock'].'&nbsp;'.$item['price'].'<br>';
                $str .= "<tr><td><input type='hidden' name='gid[]' value='".$item['id']."'</td><td>".$item['attr']."</td><td><input type='text' name='price[]' value='".$item['price']."'></td><td><input type='text' name='stock[]' value='".$item['stock']."'></td></tr>";
            }
            $str .='</table>';
...
//        $
    ...
    {//商品单位
        ...
        $key = $param['key'];
        $whereattr['attr'] = $key;
        $temp = Db::name('goods_attribute')->where($whereattr)->find();
        $pid = $temp['id'];
        $where['pid'] = $pid;
//        if (Request::isAjax()) {
        $list = Db::name('goods_attribute')
            ...
            ->select()
            ->toArray();
        $result = ['code' => 0, 'msg' => '获取成功!', 'attribute' => $list, 'rel' => 1];
        echo json_encode($result);
        exit();
//        }
    }
    ...
    public function tttttt(){
        ...
        for($k=0;$k<$num;$k++){//组成属性组 方便提交数据库
            $newarr[$k]['attr'] = $json[$k];
            $newarr[$k]['stock'] = $stock[$k];
            $newarr[$k]['price'] = $price[$k];
        }

        $result = ['code' => 0, 'msg' => '获取成功!', 'attribute' => $newarr, 'rel' => $json];
        echo json_encode($result);
    }

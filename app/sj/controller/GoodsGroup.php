...
class GoodsGroup extends BaseController
{
    ...
    function __construct(App $app)
    {
        ...
        $this->table = 'goodsgroup';
    }

    public function goodsgroupList()//商品列表
    {
        $this->ifLogin();
        if (Request::isAjax()) {
    ...
    public function goodsGroupCate()//选单分类
    {
        $this->ifLogin();
        if (Request::isAjax()) {
            $table = str_replace("List", '', __FUNCTION__);
...
//        }
    ...
    public function addGoodsGroup()
    {
        ...
        $this->ifLogin();
        $id = Request::param('id');
        Request::param('id') ? $data = $res->getOne($id) : $data = \app\sj\model\common::key_value_array_flip(config::get('wushi.goodsGroup_table'));
        if(!@$data['image']){

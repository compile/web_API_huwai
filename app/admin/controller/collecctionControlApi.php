...
define("DBUSER","collection");
define("DBPASSWD","mrcomdcea");
define("DBNAME","collection");

 **/

define("DBHOST","127.0.0.1");
define("DBUSER","bbguh");
define("DBPASSWD","BF3zzLbmA7LMnASM");
define("DBNAME","bbguh");
...
function getTypeName($type){
    /**
     * 分类
     * 娱乐 19 文化 12 科技 30 财经没有 历史 13 母婴 26 时尚 23 汽车 18  旅游 29 健康 24 军事 10 美食 28 星座 27 游戏 42 搞笑 45 动漫 41 教育 25
     */
...
function postCurl($ArticleData){
    ...
    # Create a connection
    $url = 'https://www.mrttw.com/AddNewData.php';
    $ch = curl_init($url);
    # Form data string
...
function filterContentAgain($content){
    ...
    //$content = preg_replace('/src="(?!https:\/\/i2.mrttw.com)([^"]+)"/','',$content);//清除多余的图片地址
    $content = str_replace("<span>返回搜狐，查看更多</span>","",$content);//发现未正确闭合的标签，此处清理
    $content = str_replace("<p>责任编辑：</p>","",$content);
    $content = str_replace('<p data-role="editor-name">责任编辑：<span></span></p>',"",$content);
    return $content;
}
...
function getImg($url = "", $filename = "")
{
    ...
    curl_setopt($hander, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0);
    curl_setopt($hander, CURLOPT_HTTPHEADER, array('Expect'=>''));
...
/**
 *
 * 给定文本，获得key 和 descript
 *
 */

function fenci_keyword_descript($article)
{
    $memory_info = '';
    ...
    if ($str != '') {
        //岐义处理
        $do_fork = empty($_POST['do_fork']) ? true : false;
        //新词识别
        $do_unit = empty($_POST['do_unit']) ? true : false;
        //多元切分
        $do_multi = empty($_POST['do_multi']) ? true : false;
        //词性标注
        $do_prop = empty($_POST['do_prop']) ? true : false;
        //是否预载全部词条
        $pri_dict = empty($_POST['pri_dict']) ? true : false;
        ...
        $pa->GetFinallyResult();//这是整理结果



        $row['keyword'] = $pa->GetFinallyKeywords(10);



        //截取第一段
        $newContent = trim(strip_tags($article));
        $patternArr = array('/\s/', '/ /');
        $replaceArr = array('', '');
        $newContent = preg_replace($patternArr, $replaceArr, $newContent);

        $row['descript'] = mb_strcut($newContent, 0, 400, 'utf-8');
